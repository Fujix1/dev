...
class ListView {
  ...
  async init() {
    ...
    dataIndex = Database.indexOfZip(this.zipName);
  ...
  filter() {
    var tick = Date.now();
    this.database.filter({ word: this.searchWord, fields: this.searchTarget });
    this.database.sort({ field: this.columns[this.orderByIndex].data, direction: this.sortDirection });
    console.log("Database.filtersort", Date.now() - tick, "ms");
