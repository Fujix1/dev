...
class ListView {
  constructor(args) {
    ...
    this.selectedRow = undefined; // 選択中の行
  ...
  async init() {
    ...
    console.log("初期表示 zipName", this.zipName);
    console.log("初期表示 dataIndex", this.dataIndex);
    this.setFocusOnItem();
  }
  ...
  makeVisible() {
    ...
    if (this.virtualIndex === -1) {
      ...
      this.selectedRow = this.ul.childNodes[0];
      console.log("再選択 this.zipName", this.zipName);
    }
  ...
  async saveSettings() {
    ...
    try {
      ...
      console.log("ListView設定 main.js に送信");
    } catch (e) {
      console.log(e);
    }
  }
