...
class ListView {
  constructor(args) {
    ...
    this.columnClickable = args.hasOwnProperty("columnClickable") ? args.columnClickable : true;
    ...
    this.onFocus =
      typeof args.onFocus === "function"
        ? args.onFocus
        : (e, index) => {
            console.log("onFocus", e, index);
          };

    // 状態保持
    this._itemCount = 0; // 項目数
    this.itemIndex = -1; // 選択中のリストビュー項目インデックス
    this._lastHeight = 0; // リサイズ前のサイズ保持
    this._rowHeight = 0; // 列の高さ
  ...
  async init() {
    ...
    this._rowHeight = li.offsetHeight;
    li.remove();
    this._lastHeight = this.main.offsetHeight;
    ...
    this.resizeObserver = new window.ResizeObserver(async (entries) => {
      if (entries[0].target == this.list) {
        ...
        if (this._lastHeight != stageHeight) {
          this._lastHeight = stageHeight;
  ...
  set itemCount(count) {
    ...
    // ステージの高さ設定
    const newHeight = this._itemCount * this._rowHeight;
  ...
  updateVirtualDoms() {
    ...
    let neededRows = Math.ceil(this._lastHeight / this._rowHeight) + 1; // 必要な要素数
    ...
    if (numDOMs < neededRows) {
      for (let i = numDOMs; i < neededRows; i++) {
        ...
        li.addEventListener("focus", async (e) => await this.onFocusHandler(e));
  ...
  async updateRowDOMs() {
    // DOM 項目の再配置
    const start = Math.floor(this.list.scrollTop / this._rowHeight);
    ...
    for (let i = start; i < end; i++) {
      ...
      const newTop = i * this._rowHeight + "px";
  ...
  async onFocusHandler(e) {
    // 行でイベント発生
    this.onFocus(e, e.target.virtualIndex);
  ...
  async makeVisible(reselect = true) {
    // 0件のとき
    if (this._itemCount === 0) {
      this.itemIndex = -1;
      this.onSelect(-1);
    ...
    const newPos = this.itemIndex * this._rowHeight;
    if (
      this.list.scrollTop < newPos &&
      this.list.scrollTop > newPos - this.list.offsetHeight + scrollBarHeight + this._rowHeight * 2
    ) {
      // 表示範囲内
    } else {
      // 表示範囲外
      if (this.list.scrollTop < newPos) {
        // 下向きスクロール
        this.list.scrollTop = newPos - this.list.offsetHeight + this.header.offsetHeight + this._rowHeight;
      } else {
        this.list.scrollTop = newPos;
      }
      await this.updateRowDOMs();
  ...
  updateRowTexts() {
    for (let i = 0; i < this.ul.childElementCount; i++) {
      this.updateRow(this.ul.children[i]);
    }
  }
  ...
  updateRow(li) {
    ...
    for (let i = 0; i < this.columns.length; i++) {
      li.children[i].innerHTML = row[this.columns[i].data];
  ...
  setSortArrow(orderByIndex, sortDirection) {
    if (this.columnClickable === false) return;
