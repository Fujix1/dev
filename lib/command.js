...
class Command {
  ...
  async init(args) {
    ...
    fileContents.split(/\r?\n/).forEach((line) => {
      if (line.startsWith("$info=")) {
        if (items.length > 0) {
          let lastPage = 0; // 最後に選択したページの復旧
          if (this.config.history.hasOwnProperty(info[0])) {
            lastPage = this.config.history[info[0]];
          }
          this.master[n] = { pages: items, info: info[0], lastPage: lastPage };
          info.forEach((item) => {
    ...
    if (items.length > 0) {
      ...
      this.master[n] = { pages: items, info: info[0], lastPage: 0 };
    }
    ...
    // オプション
    document.getElementById("flipCommand").checked = this.config.flip;
    document.getElementById("wrapCommand").checked = this.config.wordwrap;
    document.getElementById("zenhanCommand").checked = this.config.zenhan;
    document.getElementById("flipCommand").addEventListener("change", (e) => {
      this.config.flip = document.getElementById("flipCommand").checked;
      this.showEntry();
    });
    document.getElementById("wrapCommand").addEventListener("change", (e) => {
      this.config.wordwrap = document.getElementById("wrapCommand").checked;
      this.doWrap();
    });
    document.getElementById("zenhanCommand").addEventListener("change", (e) => {
      this.config.zenhan = document.getElementById("zenhanCommand").checked;
      this.showEntry();
    });
  ...
  show(zipName) {
    if (zipName === "") {
      ...
      // 親セットで再確認
      const dataIndex = Dataset.master.findIndex((e) => e.zipname === zipName);
      const masterId = parseInt(Dataset.master[dataIndex].masterid);
      if (masterId === -1) {
        this.clear();
        return;
      } else if (!(Dataset.master[masterId].zipname in this.table)) {
        this.clear();
        return;
      } else {
        zipName = Dataset.master[masterId].zipname;
      }
    }
