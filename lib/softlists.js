...
class Softlists {
  ...
  async init() {
    ...
    this.dropdown.addEventListener("keydown", (e) => {
      if (e.shiftKey && e.key == "Tab") {
        listViewMain.list.focus();
        e.preventDefault();
      }
    });

    // タブ切り替えで検索有効化
    document.querySelectorAll(".m-tab--bottom input").forEach((item) => {
      item.addEventListener("change", (e) => {
        this.toggleSearchSoft();
      });
    });
  }
  ...
  saveSettings() {
    ...
    for (let i = 0; i < this.history.length; i++) {
      if (this.lastSelects.hasOwnProperty(this.history[i])) {
        lastSelectsEx[this.history[i]] = this.lastSelects[this.history[i]];
      }
    }

    const settings = {
      history: this.history,
      lastSelects: lastSelectsEx,
    };
    try {
  ...
  async showEntry(newSoftlist, focusItem = false, selectedByUser = false) {
    ...
    if (this.currentSoftlist !== newSoftlist) {
      ...
      // データ準備
      this.filter({ word: config.searchWordSoft });
      this.sort({
  ...
  toggleSearchSoft() {
    if (document.querySelector(".m-tab--bottom input[value='1']").checked && softlists.currentSoftlists.length > 0) {
      document.querySelector(".p-search--soft").classList.remove("disabled");
      document.getElementById("searchSoft").disabled = false;
    } else {
      document.querySelector(".p-search--soft").classList.add("disabled");
      document.getElementById("searchSoft").disabled = true;
    }
  }
}
