...
'use strict';
{
  ...
  window.addEventListener('DOMContentLoaded', async (e) => {
    ...
    document.querySelectorAll('[tooltip]').forEach(item => {
      // position: static; のときは relative にする
      if (window.getComputedStyle(item).getPropertyValue('position') === 'static') {
      ...
      item.addEventListener('mousemove', e=>{
        ...
        if ( e.target.tipson === false ) {
          e.target.hTimeout =  setTimeout( () => {
            ...
            span.style.left = (e.offsetX + 10) + "px";
            span.style.right = null;
            span.style.top = (e.offsetY + 14) + "px";
