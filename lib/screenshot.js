...
class ScreenShot {
  ...
  async show(zipName) {
    ...
    const masterId = Dataset.master[dataIndex].hasOwnProperty("masterid") ? Dataset.master[dataIndex].masterid : -1;
    ...
    if (!result.result && masterId !== -1) {
      const masterZipName = Dataset.master[masterId].zipname;
    ...
    if (result.result) {
      ...
      window.requestAnimationFrame(async () => {
        document.querySelector(".p-info__img").setAttribute("src", "data:" + this.type + ";base64," + result.img);
        //document.documentElement.style.setProperty("--screenshot", "url(data:" + this.type + ";base64," + result.img);
        this.setAspect();
      });
    } else {
      this.clear();
  ...
  async clear() {
    ...
    //document.documentElement.style.removeProperty("--screenshot");
  }
  ...
  async setAspect() {
    ...
    if (this.keepAspect) {
      let aspectX, aspectY;
      if (Dataset.master[this.index].vertical) {
        aspectX = "3";
        aspectY = "4";
      } else {
        aspectX = "4";
      ...
      // 特殊画面比率
      // 横3画面と2画面 (ギャップ対応)
      const OrgResX = Dataset.master[this.index].resx;
      const OrgResY = Dataset.master[this.index].resy;
      const NumScreens = Dataset.master[this.index].numscreens;
