...
class Action {
  constructor(args) {
    ...
    if (this.keycode) sc.push(this.keycode);
    ...
    this.li.addEventListener("mousedown", (e) => {
      if (this.enabled) {
        ...
        console.log("execute");
        this.execute(e);
        if (!this.parent) PopupMenu.close(); // 親じゃなければ閉じる
...
class PopupMenu {
  ...
  async show(e) {
    ...
    // メニューを作る
    this.addItems(e, this.ul, this.items, this);
  ...
  // 該当 メニューの項目を非選択状態にする
  unHighlight(targetUL) {
    targetUL.removeAttribute("activeIndex");
