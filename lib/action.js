...
class Action {
  ...
  constructor(args) {
    ...
    this.li.addEventListener("mouseup", async (e) => {
      if (this.enabled) {
        // 右クリックのとき
        if (e.button === 2) {
  ...
  async execute(e) {
    ...
    if (this.enabled) {
      if (typeof this.onExecute === "function") {
        PopupMenu.close();
        document.body.classList.remove("was-mainmenu-open");
        if (e.type === "keydown" && this.isEditItem) {
          // 編集項目は二重で起こるのを防止
        } else {
  ...
  constructor(args) {
    ...
    if (args.targets) {
      for (let i = 0; i < args.targets.length; i++) {
        if (args.isMainMenu) {
          // メインメニュー
          document.querySelector(args.targets[i]).addEventListener("mousedown", (e) => {
            e.stopPropagation();
            PopupMenu.close(); // 既存を閉じる
            if (document.body.classList.contains("was-mainmenu-open")) {
              document.body.classList.remove("was-mainmenu-open");
            } else {
              this.caller = e; // 呼び出しイベント
              PopupMenu.nextPopup = null;
              document.body.classList.add("was-mainmenu-open");
              this.realShow(null);
            }
          });
        } else {
...
const actMinimize = new Action({
  ...
  control: true,
  onExecute: async (self) => {
    // this doesn't work with a "mousedown" event
...
const actAbout = new Action({
  caption: "バージョン情報...",
  onExecute: async (self) => {
    window.retrofireAPI.showFormAbout();
  },
  onUpdate: async (self) => {
    self.enabled = true;
  },
